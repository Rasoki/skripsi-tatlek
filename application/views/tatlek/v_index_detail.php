...
//                                   print_r($item);
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <select class="form-control" id="eclat3" name="eclat3">
                                            <?php
                                            foreach($select_eclat as $key=>$val)
                                            {
                                                echo '<option value='.$val.'>'.$val.'</option>';
                                    ...
                                    <td>
                                        <select class="form-control" id="confidence3" name="confidence3">
                                            <?php
                                            foreach($select_confidence as $key=>$val)
                                            {
                                                echo '<option value='.$val.'>'.$val.'</option>';
...
//                                            }
                                ...
                                <tr>
                                <td>
                                        <select class="form-control" id="eclat4" name="eclat4">
                                            <?php
                                            foreach($select_eclat as $key=>$val)
                                            {
                                                echo '<option value='.$val.'>'.$val.'</option>';
                                    ...
                                    <td>
                                        <select class="form-control" id="confidence4" name="confidence4">
                                            <?php
                                            foreach($select_confidence2 as $key=>$val)
                                            {
                                                echo '<option value='.$val.'>'.$val.'</option>';
...
<script type="text/javascript">
   $(document).ready(function(){
       ...
       $('#eclat3,#confidence3').on('change',function(){
           getValue3();
       })
       $('#eclat4,#confidence4').on('change',function(){
           getValue4();
       })
   })
   ...
   function getValue()
   {
       ...
       $.ajax({
           type: "POST",
           dataType: 'JSON',
           data: {support: $('#eclat1').val(),confidence:$('#confidence1').val(),item:<?php echo json_encode($item); ?>},
           url: "<?php echo site_url('tatlek/itemset')?>",
   ...
   function getValue2()
   {
    $("a#detail_eclat2").attr("href", "<?php echo site_url('tatlek/detail_eclat/')?>/"+$('#eclat2').val()+"/"+$('#confidence2').val()+"");
       $.ajax({
           type: "POST",
           dataType: 'JSON',
           data: {support: $('#eclat2').val(),confidence:$('#confidence2').val(),item:<?php echo json_encode($item); ?>},
           url: "<?php echo site_url('tatlek/itemset')?>",
           success: function(data){
   ...
   function getValue3()
   {
    $("a#detail_fpgrowth1").attr("href", "<?php echo site_url('tatlek/detail_fpgrowth/')?>/"+$('#eclat3').val()+"/"+$('#confidence3').val()+"");
       $.ajax({
           type: "POST",
           dataType: 'JSON',
           data: {support: $('#eclat3').val(),confidence:$('#confidence3').val(),fpg_value:<?php echo json_encode($fpg_value); ?>,total:<?=$total;?>,data_growth:<?= json_encode($data_growth)?>},
           url: "<?php echo site_url('tatlek/itemset2')?>",
           success: function(data){
             $('#itemset3').html(data.nilai);
         }
     });    
   }
   function getValue4()
   {
    $("a#detail_fpgrowth2").attr("href", "<?php echo site_url('tatlek/detail_fpgrowth/')?>/"+$('#eclat4').val()+"/"+$('#confidence4').val()+"");
       $.ajax({
           type: "POST",
           dataType: 'JSON',
           data: {support: $('#eclat4').val(),confidence:$('#confidence4').val(),fpg_value:<?php echo json_encode($fpg_value); ?>,total:<?=$total;?>,data_growth:<?= json_encode($data_growth)?>},
           url: "<?php echo site_url('tatlek/itemset2')?>",
           success: function(data){
             $('#itemset4').html(data.nilai);
         }
     });    
   }
</script>
